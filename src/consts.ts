// Place any global data in this file.
// You can import this data from anywhere in your site by using the `import` keyword.

export const NETWORK_TITLE = 'Red Internacional de Memorias Participativas';
export const NETWORK_DESCRIPTION = 'Red de encuentro, investigación y acción sobre memorias participativas en territorio.';
export const NETWORK_EMAIL = 'simposiomemoriasparticipativas@gmail.com';
export const NETWORK_INSTAGRAM = 'https://www.instagram.com/memoriasparticipativas/';

// Valores por defecto del simposio activo (se sobrescriben dinámicamente cuando hay datos).
export const SITE_TITLE = 'Red Internacional de Memorias Participativas';
export const SITE_DESCRIPTION = NETWORK_DESCRIPTION;
export const SITE_URL = 'https://simposio-memorias-participativas.netlify.app';
export const SITE_LANG = 'es';
export const SITE_KEYWORDS = [
  'memorias participativas',
  'simposio',
  'memoria',
  'red internacional',
  'territorio',
  'investigación',
  'comunidad',
];
export const DEFAULT_OG_IMAGE = '/images/hero-bg.jpg';
export const THEME_COLOR = '#2f680c';
export const AUTHOR = 'Red Internacional de Memorias Participativas';
